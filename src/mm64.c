...
addr_t pte_get_kernel_entry(struct krnl_t *krnl, addr_t pgn)
{
  if (krnl == NULL || krnl->krnl_pgd == NULL)
    return 0;
  addr_t pgd = 0, p4d = 0, pud = 0, pmd = 0, pt = 0;
  get_pd_from_pagenum(pgn, &pgd, &p4d, &pud, &pmd, &pt);
  if (krnl->krnl_pgd[pgd] == 0)
    return 0;
  addr_t *p4d_table = (addr_t *)krnl->krnl_pgd[pgd];
  if (p4d_table[p4d] == 0)
    return 0;
  addr_t *pud_table = (addr_t *)p4d_table[p4d];
  if (pud_table[pud] == 0)
...
static addr_t *get_or_create_kernel_pte(struct krnl_t *krnl, addr_t pgd, addr_t p4d, addr_t pud, addr_t pmd, addr_t pt)
{
  if (krnl->krnl_pgd[pgd] == 0)
  {
    krnl->krnl_pgd[pgd] = (addr_t)calloc(512, sizeof(addr_t));
  }
  addr_t *p4d_table = (addr_t *)krnl->krnl_pgd[pgd];
  if (p4d_table[p4d] == 0)
  {
...
int pte_set_kernel_entry(struct krnl_t *krnl, addr_t pgn, addr_t pte_val)
{
  addr_t pgd = 0, p4d = 0, pud = 0, pmd = 0, pt = 0;
  get_pd_from_pagenum(pgn, &pgd, &p4d, &pud, &pmd, &pt);
  addr_t *pte = get_or_create_kernel_pte(krnl, pgd, p4d, pud, pmd, pt);
  *pte = pte_val;
  return 0;
}
int pte_set_kernel_fpn(struct krnl_t *krnl, addr_t pgn, addr_t fpn)
{
  addr_t pte = 0;
  SETBIT(pte, PAGING_PTE_PRESENT_MASK);
  CLRBIT(pte, PAGING_PTE_SWAPPED_MASK);
  SETVAL(pte, fpn, PAGING_PTE_FPN_MASK, PAGING_PTE_FPN_LOBIT);
  return pte_set_kernel_entry(krnl, pgn, pte);
}
...
{                                                      // no guarantee all given pages are mapped
  ...
  {
    addr_t current_pgn = pgn_start + pgit;
    pte_set_fpn(caller, current_pgn, fpit->fpn);
